...
按 SKILL 正文的 CUDA/CUTLASS 起手式,**两组分开跑**。

**第一组 —— 算数值的:**

```bash
grep -nE 'gemm\(|mma_|__expf|expf\(|rsqrtf|__shfl|ex2_approx|sigmoid' \
...
```bash
grep -nE 'cute::transform|bf16_to_f32|BF16\(|FP16\(|convert_type|NumericConverter' \
     csrc/smxx/fwd_kernel2.cuh
```

MMA warp(`:520-730`)里 `return BF16(x)` 恰好 **4 处**:`:570`、`:591`、`:618`、`:644`。同一区间的 `gemm(thr_mma, ..)` 有 **7 发**:`:548,549,562,563,616,641,694`。

逐条认领(这就是 SKILL §四 那个核对):

| 降档 | 降的是谁 | 类型 |
|---|---|---|
| `:570` | `:562,563` 攒出的 o | 落盘(存 out_bf16) |
| `:591` | `:548,549` 攒出的 K̃S | **接力** → 喂 `:616` 的 B |
| `:618` | `:616` 的 U | **接力** → 喂 `:641` 和 `:694` 的 B |
| `:644` | `:641` 的 o += Mqk@U | 落盘(写回 GM) |
| `:714-715` | `:694` 的 ΔS | 落盘(存回 BF16 状态)—— **正则没抓到,是倒着查 `:694` 才补上的** |

**接力那两条一条都不能少** —— `:616`/`:641`/`:694` 的 B 操作数必须 BF16,FP32 累加器喂不进去。落盘那三条则是选择。

**注意最后一行**:`:714-715` 的降档内联在状态递推表达式里(`BF16(bf16_to_f32(S)*g + u_acc)`),不是独立的 `cute::transform`,上面那条正则漏掉了它。这正是 SKILL §四 说"从 GEMM 那侧倒着查"的原因 —— 按 grep 结果正着查,你不会发现少了一条;按"每发 GEMM 的结果去哪了"查,`:694` 立刻逼你找出它的出口。

**这一组必须单独成表,不能和第一组合着看** —— 合着看它们就是 20 多条里混着的几行,长得像搬运,一眼扫过去就跳过了。单独列出来,你才会去填上面那张认领表。这份材料第一版正是把两组合成一条正则跑的,`:618` 就那样漏了(见四(d))。
...
| 记号 | 出现在 | 出处 |
|---|---|---|
| `[bf16·bf16→f32→bf16]` | K2 里结果要接力或落盘的 GEMM | atom `SM80_16x8x16_F32BF16BF16F32_TN`;降档在 `fwd_kernel2.cuh:570,591,618,644` |
| `[fp16·fp16→fp16]` | 只有求逆 | `SM80_16x8x16_F16F16F16F16_TN`,`utils.cuh:200` |
| `[bf16→f32 FMA→bf16]` | 只有状态递推 | 升 `fwd_kernel2.cuh:714-715`,存回 `:82` |

(`:694` 那发的 ΔS 也降了 BF16,但降档写在 `:714-715` 的状态递推表达式里 —— `BF16(bf16_to_f32(S)*g + u_acc)` —— 不是一条独立的 `cute::transform`,所以上面那条 `return BF16(x)` 的正则**抓不到它**。这提醒一件事:第二组正则也会漏,**认领表比正则更权威** —— 从"每发 GEMM 的结果去哪了"倒着查,才不会依赖正则写得全不全。)
...
**查法**:把降档语句单独 grep 成一组(见二),再逐条认领它降的是哪一发 GEMM —— 认领表见二。`:618` 属于"接力"那一类,漏了就必然在流上留下一个断口:⑩ 出口标 FP32、⑪⑫ 入口是 BF16,对不上。**这个断口就是 §七 第 4 条"精度闭合"要查的东西**。
