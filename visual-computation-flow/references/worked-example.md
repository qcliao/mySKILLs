...
| 记号 | 出现在 | 出处 |
|---|---|---|
| `[bf16·bf16→f32]` | K2 的 5 发 GEMM,全部 | `SM80_16x8x16_F32BF16BF16F32_TN`,`fwd_kernel2.cuh:465` |
| `[fp16·fp16→fp16]` | 只有求逆 | `SM80_16x8x16_F16F16F16F16_TN`,`utils.cuh:200` |
| `[bf16→f32 FMA]` | 只有状态递推 | `fwd_kernel2.cuh:714-715` |
...
2. **⑫ 是全仓唯一的 FP32 FMA**:state 存 BF16(`fwd_kernel2.cuh:82`)但递推那一步升回 FP32 算。"存储降档"和"计算降档"是两件事。
